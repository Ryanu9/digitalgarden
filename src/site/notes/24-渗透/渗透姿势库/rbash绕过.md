...
ssh a@1.1.1.1 -t /bin/bash

```
## 3. rbash绕过方式
参考文章： https://www.freebuf.com/vuls/376922.html
### 3.1. 使用 / 符号
如果 RBASH 没有完全禁止使用 / 符号，那么用户可以尝试直接运行 /bin/bash 或 /bin/sh 来获得一个正常的 shell
```bash
rbash$ /bin/bash
```
...
### 3.3. 使用 ftp, gdb, more, man, less, vim, rvim 等命令
...
### 3.5. 使用 awk, find, expect, python, php, perl, lua, ruby 等编程语言或工具
